...
function Async<T>({
  ...
  onError,
  onSuccess,
}: AsyncProps<T>): React.ReactElement {
  ...
  switch (asyncState.status) {
    ...
    case 'error':
      return typeof error === 'function' ? <>{error(asyncState.error!)}</> : <>{error}</>;
    case 'success':
      return typeof success === 'function' ? <>{success(asyncState.data!)}</> : <>{success}</>;
  }
}
