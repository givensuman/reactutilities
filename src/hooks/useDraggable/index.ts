...
function useDraggable(
    ...
    const [position, setPosition] = useState<DraggableReturn["position"]>({ x: 0, y: 0 });
      ...
      const handleMouseMove = (event: MouseEvent) => {
        ...
        if (bounds) {
          ...
          if (bounds.right !== undefined) {
            dx = Math.min(dx, bounds.right - draggable.offsetLeft - draggable.clientWidth);
          }
          if (bounds.top !== undefined) {
            dy = Math.max(dy, bounds.top - draggable.offsetTop);
          }
          if (bounds.bottom !== undefined) {
            dy = Math.min(dy, bounds.bottom - draggable.offsetTop - draggable.clientHeight);
          }
        }
    ...
    }, [containerRef, elementRef, onDragStart, onDrag, onDragEnd, axis, bounds, isDragging, draggingPosition]);
